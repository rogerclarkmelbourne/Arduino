...
prog_char soaktest_htm[] PROGMEM = "<html><head><link rel=\"stylesheet\" type=\"text/css\" href=\"s.css\"><meta http-equiv=\"refresh\" content=\"5\"></head>" 
                              "<body><h3>Millis are \a0\a</h3>"
                              "<h3>Analog A0 = \a1\a <meter min=\"0\" max=\"1023\" value=\"\a1\a\"></meter><h3>"
                              "<h3>Analog A1 = \a2\a<h3>"
                              "<h3>Analog A2 = \a3\a<h3>"
                              ...
                              "<h3>Analog A4 = \a5\a<h3>"                                    
                              "<body>";// This is the page data
prog_char soaktest_pageName[] PROGMEM =  "soaktest.htm";// This is the page name
...
{   
  ...
  {soaktest_pageName,soaktest_htm,mimetype_text_html},
 ...
 */
//int buildPage(char *page,char variables[NUMBER_OF_VARIABLES][STRLEN_OF_VARIABLES],Stream *oStream=((Stream *)0))
int buildPage(int pageIndex,char variables[NUMBER_OF_VARIABLES][STRLEN_OF_VARIABLES],Stream *oStream=((Stream *)0))
{
  ...
  char varIndex[4];
  char *varIndexPtr;
  char *page = (char*)pgm_read_word(&(pages[pageIndex][1]));//(char *)pages[pageIndex][1];
...
// Setup all the variables that will be used on the page (Note make sure your array is big enough in the #define 
 ...
 // Iterate through the pages list to find a name that matches
    ...
    {
      if (strcmp_P(pch,(char*)pgm_read_word(&(pages[page][0])))==0)
      {
        foundPage=page;
        break;
      }
...
// Send the response back to the module
  ...
  printP(serialPort,(char*)pgm_read_word(&(pages[foundPage][2])));
  serialPort->println();
